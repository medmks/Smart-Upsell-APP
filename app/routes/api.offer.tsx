...
import { getOffers,getRecomendedOffers } from "../offer.server";
...
export const action:ActionFunction = async ({ request }) => {
   ...
   const offers = await getOffers(sessionToken?.input_data?.shop?.domain);
     const recomendedoffers = await getRecomendedOffers("gid://shopify/Product/9085500457235");

  return cors(json({ offers,recomendedoffers }));
};
